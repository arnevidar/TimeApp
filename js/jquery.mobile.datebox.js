    ...
    $.widget( "mobile.datebox", $.mobile.widget, {
        ...
        _update: function() {
            ...
            if ( o.mode === 'calbox' ) { // Meat and potatos - make the calendar grid.
                ...
                /**
                 * A hack to get it to work, since this plugin doesn't support isodates coming from
                 * objects and variables, only as plain string arrays.
                 */
                var finishedDatesLS = localStorage.getItem("finishedDates");
                var finishedDatesLSA = finishedDatesLS.split(",");

                var notFinishedDatesLS = localStorage.getItem("notFinishedDates");
                var notFinishedDatesLSA = notFinishedDatesLS.split(",");
                ...
                //alert("THIS IS o.finishdates "+ o.finishedDates);

                //alert("THIS IS o.blackDates AFTER: " + o.blackDates);
                    ...
                    if ( gridWeek === 0 || ( gridWeek > 0 && (calmode.today > 0 && calmode.today <= calmode.end) ) ) {
                        ...
                        for ( gridDay=0; gridDay<=6; gridDay++) {
                            ...
                            if ( calmode.today < 1 ) {
                                ...
                                } else if ( $.isArray(finishedDatesLSA) && ($.inArray(self._isoDate(self.theDate.getFullYear(), self.theDate.getMonth()+1, calmode.today), finishedDatesLSA) > -1 )  ) {
                                    calmode.thisTheme = o.finishedDatesTheme;
                                } else if ( $.isArray(notFinishedDatesLSA) && ($.inArray(self._isoDate(self.theDate.getFullYear(), self.theDate.getMonth()+1, calmode.today), notFinishedDatesLSA) > -1 ) ) {
